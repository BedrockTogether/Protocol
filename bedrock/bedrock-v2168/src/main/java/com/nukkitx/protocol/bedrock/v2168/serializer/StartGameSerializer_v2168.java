...
/**
 * Extends v924 rather than v1001 on purpose (upstream does the same, skipping its
 * own v944): v1001 is the version that introduced the isLoggingChat flag 1.26.40
 * removed, so inheriting from it would risk picking that flag back up. The two
 * fields v1001 legitimately added are written inline below instead.
 * <p>
 * Level settings are rewritten in full rather than delegated because three fields
 * changed encoding inside the block:
 * <ul>
 *     <li>eduEditionOffers: varint -&gt; unsigned varint</li>
 *     <li>game rules: integer values are little endian ints now
 *     (see {@link com.nukkitx.protocol.bedrock.v2168.BedrockPacketHelper_v2168})</li>
 *     <li>defaultPlayerPermission: varint -&gt; byte</li>
 * </ul>
 */
...
public class StartGameSerializer_v2168 extends StartGameSerializer_v924 {
    ...
    protected void readLevelSettings(ByteBuf buffer, BedrockPacketHelper helper, StartGamePacket packet) {
        ...
        packet.setDefaultPlayerPermission(PLAYER_PERMISSIONS[buffer.readUnsignedByte()]);
        ...
        //v1001
        VarInts.readInt(buffer); // getServerEditorConnectionPolicy
        buffer.readBoolean(); // isAllowAnonymousBlockDropsInEditorWorlds
    }
